...
- [ ] Build signed release candidates and a rollback-capable patch pipeline. (A pinned SemVer/build contract now validates the Windows x86_64, macOS universal, and Linux x86_64 presets against project and CI engine metadata. A manual, read-only CI workflow provisions matching templates, runs all validation, exports every target, audits their executable/container structures, launches the exported Linux package smoke, packages byte-deterministic unsigned archives, strictly re-verifies them, and uploads a 14-day artifact. Embedded/outer SHA-256 manifests bind source configuration, artifact-audit logic, and both workflows; self-tests reject contract/pipeline drift and tampering. A separate offline channel archives complete candidates, atomically promotes an active pointer, preserves a contiguous transition history, permits rollback only to a previously active candidate, and re-verifies old package bytes without depending on the current source version. A real local unsigned candidate and channel promotion pass have been exercised. Owned platform identities, signing/notarization credentials, protected long-term artifact hosting, post-signing provenance, hosted staged rollout, and delta-patch delivery remain external production gates.)
