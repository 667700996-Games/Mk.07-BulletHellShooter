...
The production quality plan and milestone gates are tracked in `docs/AAA_UPGRADE_ROADMAP.md`. Human balance sessions use the privacy-safe protocol and aggregate release gate in `docs/PLAYTEST_PROTOCOL.md`; automated runs do not count as player evidence. Source-art and runtime-source ceilings, naming rules, decoded texture estimates, and required asset references are defined in `docs/CONTENT_BUDGETS.md` and enforced in CI.
